...
const ROTATION_END_ANGLE = 211.5; // İbrenin ulaşabileceği maksimum açı
const PERCENTAGE_DIVISOR = ROTATION_END_ANGLE / 2; // Yüzdelik değeri açıya çevirmek için çarpan

// Eylemsizlik (Inertia) Sabitleri (Değişimlerin yumuşaklığını belirler)
...
const FUEL_CONSUME_TICK_MS = 1000;     // Döngü güncelleme hızı (1 saniye)
const KG_CONVERSION = 0.453592; // LB biriminden KG birimine çevrim katsayısı
...
let targetOilQtyLeft = 18.0; // Yağ sızıntısı simülasyonu için hedef seviye
let targetOilQtyRight = 18.0;

let currentThrottleTargetLeft = 0;
let currentThrottleTargetRight = 0;
let currentThrottleLeft = 0; // Eylemsizlik uygulanmış anlık gaz kolu pozisyonu
...
// İlk açılışta EICAS arızalarının doğrudan yanmasını engelleyen bloklayıcı
...
// Uygulama yüklendiğinde gizleme işlemini başlat
resetSystemUI();

// Arayüz geçiş (toggle) araçlarının başlangıç durumlarını ayarlar
document.addEventListener('DOMContentLoaded', () => {
    // Yakıt birimi: LB olarak başlat
    ...
    // Sıcaklık birimi: C olarak başlat
...
// Belirtilen elementlerin görünürlük (display) durumunu tersine çevirir
...
// Belirtilen ID'ye sahip SVG veya HTML metin elementinin içeriğini değiştirir
function updateTextContent(id, value) {
    const el = document.getElementById(id);
    if (el) el.textContent = value;
}

// İbrelerin (needle) açısını ve taradıkları (sweep) alanı günceller
function updateRotationAngle(id, angle, sweepId) {
    const el = document.getElementById(id);
    if (el) el.style.transform = `rotate(${angle}deg)`;

    // İbrenin taradığı alanı (sweep) SVG stroke-dashoffset ile dinamik olarak doldurur
    if (sweepId) {
...
function checkEngineLimits(side, n1, n2, egt, vib, press, temp) {
    // İç fonksiyon: Parametrelere göre Amber (Sarı) veya Red (Kırmızı) sınıfları uygular
    function applyColorClass(textId, needleId, value, limitAmberTop, limitRedTop, limitAmberBot, limitRedBot) {
        ...
        // Renklendirme sınıflarını (CSS class) uygula
        if (isRed) {
    ...
    // Motor Verileri Limit Uygulamaları
    ...
    // Titreşim (VIB) Inverted (Ters Renk) Durumu
...
// İtme gücüne (throttle) dayalı ana EICAS animasyon ve hedef hesaplama çerçevesi
function updateEngineState(throttleValue, side) {
    const throttle = Math.min(parseFloat(throttleValue), 105); // Fiziksel limit
    const n1 = throttle;
    const ff = throttle * 14.2; // Fuel Flow (Yakıt Akışı) hesaplaması
    const vib = throttle * 0.015; // Titreşim tahmini
    
    // Toplam Hava Sıcaklığı (Total Air Temperature) simülasyonu
    const tat = -12 + (throttle * 0.1); 
    updateTextContent('val_tat', tat.toFixed(1));

    // Motor tepki hedefleri
    ...
    const oilLossRatio = Math.max(0, (18.0 - qty) / 18.0); // Yağ sızıntısı durumunda etki çarpanı
    
    targetOilTemp += (45 * oilLossRatio);   // Yağ azaldıkça sıcaklık artar
    ...
    if (side === 'L') {
        ...
        // Sol Motor Arızalarının hedef değerlere etkisi
        ...
        // Atalet (Inertia) uygulanarak değerlerin aniden değil, yumuşak bir ivmeyle hedefe ulaşması sağlanır
    ...
    // Sağ Motor İşlemleri
        ...
        // Sağ Motor Arızalarının hedef değerlere etkisi
...
function triggerEICASWarning(elementId, activate) {
    ...
    // Metin elementiyse farklı, kutu ise farklı renk sınıfı seçilir
    ...
    if (activate) {
        ...
        // 10 saniye sonunda flaşlama durur, uyarı sabit (steady) duruma geçer
...
// Kullanıcı arayüzünden tetiklenen spesifik arızaları aktifleştirir veya kapatır
function toggleFailure(type, side) {
    if (type === 'eng') {
        ...
        // Sol veya sağ tankın yakıtını aniden limitin altına düşürür
    ...
    // Yağ basıncı uyarısını güncel duruma göre tetikle
...
// Aktif uçuş modunu (TO, CLB, CRZ vb.) ekranda ayarlar
...
// N1 referans ibresini (bug) ayarlanan değere göre konumlandırır
...
function updateFuelDisplay() {
    // Seçili birime göre değerleri dönüştürür
    ...
    // Dairesel barlar için SVG yay matematiği
    const circumference = 2 * Math.PI * 5.4;
    const sweepAngle = 261; 
    const visibleCircumference = circumference * (sweepAngle / 360);

    // Barların seviyeleri her zaman tank kapasitelerine oranlanarak hesaplanır
...
// Yakıt tüketimini simüle eder, Merkez tank bittiğinde kanat tanklarına geçer
...
// Otomatik gelişen sistem uyarılarını kontrol eder (Düşük yakıt, yağ basıncı vb.)
...
// Saniyede 60 kare (requestAnimationFrame) çalışan ana motor fiziği ve animasyon döngüsü
function engineLoop() {
    // Yakıt yoksa veya motor arızalıysa itme gücü (throttle) yaratılmaz
    ...
    // Hedef güce atalet (inertia) yardımıyla pürüzsüz geçiş 
    ...
    // Arayüzdeki slider çubuklarını her karede motorun eylemsizlik değerine göre senkronize et
    ...
    if (isScenarioRunning) {
        // SENARYO MODU: Slider'lar hedefe doğru ivmelenmeyi yansıtarak görsel olarak hareket eder
        ...
        // MANUEL MOD: Slider'lar sadece komut verilen hedefte sabit kalır (titremeyi önler)
...
// JS üzerinden slider'ları ve motor gücünü güvenle güncellemek için yardımcı fonksiyon
...
// Senaryo başlangıcında açık olabilecek tüm arızaları temizler
...
// Devam eden veya sıraya alınmış senaryoyu iptal eder
...
// Seçilen uçuş senaryosunu (Örn: Normal Kalkış, Motor Arızası) zaman çizelgesine göre oynatır
function playScenario(type) {
    ...
    // T=0 (Başlangıç Ayarları: Sistem temizlenir ve motorlar rölantiye alınır)
    resetSystemsForScenario();
    setFlightMode('NONE');
    setThrottleCommand('L', 20); 
    ...
    if (type === 'normal_takeoff') {
        schedule(3, () => {
            setThrottleCommand('L', 50); // Motor Stabilizasyonu (Güç artışı)
            setThrottleCommand('R', 50);
        });
        schedule(8, () => {
            setFlightMode('mode_to'); // Kalkış Gücü (Takeoff Power)
            setThrottleCommand('L', n1ReferenceValue);
            setThrottleCommand('R', n1ReferenceValue);
        });
        schedule(40, () => {
            setFlightMode('mode_clb'); // İtme Gücü Düşüşü (Thrust Reduction / Climb)
            setThrottleCommand('L', 88);
            setThrottleCommand('R', 88);
            clearScenario(); // Senaryo tamamlandı
        });
    } 
    else if (type === 'v1_cut') {
        schedule(3, () => {
        ...
        schedule(25, () => {
            // V1 Cut - Kritik hızda sağ motorda aniden güç kaybı arızası tetiklenir
            if (!isEngineFailRight) toggleFailure('eng', 'R'); 
        });
        schedule(40, () => {
            setFlightMode('mode_con'); // Kalan tek motor için Sürekli Max Güç (Continuous)
            setThrottleCommand('L', n1ReferenceValue); 
            clearScenario();
        });
    }
...
// Yakıt tüketim döngüsünü ve Fizik Simülasyonunu başlatıyoruz.
